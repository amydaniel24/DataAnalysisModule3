...
FROM (SELECT order_id, SUM(quantity) AS total_items
  FROM order_items
  WHERE order_id IN ( SELECT order_id  FROM `orders`  WHERE status = 'paid' )
  GROUP BY order_id) AS paid_order_totals;
...
HAVING COUNT(*) = (SELECT MAX(day_count) FROM (SELECT DAYNAME(order_datetime) AS day_name, COUNT(*) AS day_count FROM `orders` WHERE status = 'paid' GROUP BY DAYNAME(order_datetime)) AS day_counts);
...
SELECT
  (SUM(CASE WHEN payment_method = 'app' THEN 1 ELSE 0 END) / COUNT(*)) * 100

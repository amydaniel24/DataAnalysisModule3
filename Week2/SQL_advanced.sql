...
WHERE o.status = 'paid'
GROUP BY o.order_id, c.last_name, c.first_name, s.name, o.order_datetime, o.store_id
HAVING SUM(oi.quantity * p.price) >
(
  SELECT AVG(store_order_totals.order_total)
  FROM (
    SELECT
      o2.order_id,
    ...
    WHERE o2.status = 'paid'
      AND o2.store_id = o.store_id   -- correlated to outer query's store
...
ranked AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY product_revenue DESC, product_name
    ) AS rn
...
SELECT
  ...
  i.on_hand,
  su.total_units_sold,
  (su.total_units_sold - i.on_hand) AS units_gap
FROM inventory i
JOIN sold_units su
  ON i.store_id = su.store_id
 AND i.product_id = su.product_id
JOIN stores s   ON i.store_id = s.store_id
JOIN products p ON i.product_id = p.product_id
WHERE i.on_hand < su.total_units_sold
